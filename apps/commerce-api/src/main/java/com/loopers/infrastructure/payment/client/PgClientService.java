...
/**
 * PG Client를 호출하는 서비스
 * - CircuitBreaker: 외부 시스템 장애 시 차단
 * - Retry: 일시적 실패 시 재시도
 * - Fallback: 최종 실패 시 대체 응답
 */
@Service
public class PgClientService {
    ...
    /**
     * 결제 요청 실패 시 Fallback
     * - PG 시스템 장애 시 PENDING 상태로 반환
     * - 나중에 결제 상태 확인 API로 재확인 가능
     */
    private PgClient.ApiResponse<PgClientDto.TransactionResponse> requestPaymentFallback(
        ...
        return new PgClient.ApiResponse<>(true, fallbackResponse, null, null);
    }
